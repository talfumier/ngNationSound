...
export class ProgramComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  getFormattedData(evts: any[]) {
    evts = evts.map((evt: any) => {
      //populate events with artist and location data
      return {
    ...
    let artistEvts: ArtistEvents = {} as ArtistEvents,
      i = null;
    evts.map((evt: any) => {
      i = -1;
      ...
      artistEvts.dates.push({
        date: evt.date,
        location: evt.location as Poi,
        type: evt.type,
      });
