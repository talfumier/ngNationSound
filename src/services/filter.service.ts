...
import { addDays, format, addHours, parse } from 'date-fns';
...
export class FilterService {
  ...
  setFormFilterElements() {
    //initialize form filter elements in program page and set default filter
    const days = [];
    ...
    const opts: Option[] = [],
      obj = { id: -1, name: '' }; //obj is the default option
    new Array(13)
      .fill(11)
      .map((item, idx) =>
        opts.push({ id: `${item + idx}h00`, name: `${item + idx}h00` })
      );
  ...
  filterEvent(dates: any, event: Event): boolean {
    ...
    Object.keys(dates).map((key: string) => {
      if (
        key !== 'time' &&
        !bl[0] &&
        evtDate >= addHours(dates[key], dates.time.min) &&
        evtDate <= addHours(dates[key], dates.time.max)
      )
        bl[0] = true;
    });
    if (!bl[0]) return false;
    // dealing with 'Quoi ?' filter
    if (this._filter.types['all' as keyof object]) bl[1] = true;
    else if (!bl[1]) {
      Object.keys(this._filter.types).map((key: any) => {
        if (
          key !== 'all' &&
          !bl[1] &&
          this._filter.types[key as keyof object] &&
          event.type === key
        )
          bl[1] = true;
      });
    ...
    if (
      ...
      this._filter.artist['id' as keyof object] === ''
    )
      bl[2] = true;
  ...
  setFilteredEvents(): void {
    ...
    if (!this._filter.days['all' as keyof object]) {
      //when all:true, keep all dates
      Object.keys(this._filter.days).map((key: string) => {
        //dealing with "Quand ?" filter
        switch (key) {
          case 'all': //do nothing
            break;
          default: //remove date when filter 'dayx' is false
            if (!this._filter.days[key as keyof object]) delete dates[key];
        }
      });
    }
    ['min', 'max'].map((key) => {
      //dealing with 'A quelle heure ?' filter
